...
type Manager interface {
	...
	RunReconciler(ctx context.Context, coord CoordClient, allocVersion SnapshotVersionAllocator) error
...
type collectionState struct {
	...
	principalTags                     map[string]map[string]string
	compiled                          map[compiledKey]*compiledExpression
}
...
func (m *manager) ensureFreshMetadata(ctx context.Context, collectionID UniqueID) error {
	...
	_, err, _ := m.metadataRefreshes.Do(strconv.FormatInt(collectionID, 10), func() (struct{}, error) {
		...
		if err := m.refreshCollectionSnapshot(ctx, coord, "", "", collectionID, version); err != nil {
...
func (m *manager) setRLSPolicySnapshot(_ string, collectionID UniqueID, snapshot policySnapshot) bool {
	...
	key := newCollectionKey(collectionID)
	state := m.getOrCreateCollectionState(key)
	state.mu.Lock()
	...
	state.compiled = map[compiledKey]*compiledExpression{}
...
func (state *collectionState) getCompiledExprAndTags(principalName string, action rlsutil.PolicyAction, kind exprKind, schemaHelper *typeutil.SchemaHelper, visitorArgs *planparserv2.ParserVisitorArgs, exprSelector func(*rlsutil.RowPolicy) string) (*compiledExpression, map[string]string, error) {
	key := compiledKey{
		action: action,
		kind:   kind,
	}
	...
	if compiledExpr, ok := state.compiled[key]; ok {
		tags := state.principalTags[principalName]
		state.mu.RUnlock()
		return compiledExpr, tags, nil
	}
	state.mu.RUnlock()
	...
	compiledExpr, err := compileExprTemplates(schemaHelper, templates, visitorArgs)
	if err != nil {
		return nil, nil, err
	}
	state.compiled[key] = compiledExpr
	return compiledExpr, state.principalTags[principalName], nil
}
