...
func TestCore_RLSAPIs(t *testing.T) {
	...
	policyLock.EXPECT().Broadcast(mock.Anything, mock.Anything).Return(nil, nil).Times(6)
	...
	wal := mock_streaming.NewMockWALAccesser(t)
	wal.EXPECT().ControlChannel().Return("by-dev-rootcoord-dml_0").Times(6)
	streaming.SetWALForTest(wal)
	...
	require.NoError(t, err)
	assert.True(t, merr.Ok(status))
	assert.Equal(t, 1, allocations, "exact retries must not allocate another policy ID")
