...
func (c *Core) broadcastAlterCollectionForAlterCollection(ctx context.Context, req *milvuspb.AlterCollectionRequest) error {
	...
	for _, key := range req.GetDeleteKeys() {
		if strings.EqualFold(key, common.RLSEnabledKey) && key != common.RLSEnabledKey {
			return merr.WrapErrParameterInvalidMsg("invalid property key %q, did you mean %q?", key, common.RLSEnabledKey)
		}
	}
