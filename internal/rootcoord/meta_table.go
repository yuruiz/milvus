...
func (mt *MetaTable) PrepareCreateRLSPolicy(ctx context.Context, req *rlsutil.CreateRowPolicyRequest, policyID int64) (*model.RLSPolicy, error) {
	...
	// Validate only the stable lookup bound before checking for an exact retry.
	// Refreshable creation limits must not make an existing policy impossible
	// to retry after the original response was lost.
