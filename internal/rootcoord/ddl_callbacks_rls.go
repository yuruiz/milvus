...
func (c *Core) broadcastCreateRLSPolicy(ctx context.Context, req *rlsutil.CreateRowPolicyRequest) error {
	...
	// Use an unallocated ID while checking for an exact retry. Only a genuinely new
	// policy should consume an ID, and the check must happen while holding the
	// collection guard so a concurrent RLS mutation cannot change the result.
