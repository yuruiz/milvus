...
// ValidatePolicyNameWithLimit validates a policy name for create or update.
...
// ValidatePolicy validates the structural fields of a policy definition.
func ValidatePolicy(policyName string, policyType PolicyType, actions []PolicyAction, usingExpr string, checkExpr string) error {
	if err := ValidatePolicyNameWithLimit(policyName); err != nil {
		return err
	}
	switch policyType {
